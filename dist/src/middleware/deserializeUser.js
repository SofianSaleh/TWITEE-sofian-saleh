    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                ...
                if (!accessToken)
                    return [2, next()];
                _a = (0, jwt_utils_1.verifyJwt)(accessToken, process.env.JWT1), decoded = _a.decoded, expired = _a.expired;
                if (decoded) {
                    res.locals.user = decoded;
                    return [2, next()];
                }
                if (!(expired && !!refreshToken)) return [3, 2];
                return [4, (0, session_service_1.reIssueAccessToken)(refreshToken)];
            case 1:
                newAccessToken = _b.sent();
                if (!!newAccessToken) {
                    res.setHeader('x-access-token', newAccessToken);
                    result = (0, jwt_utils_1.verifyJwt)(newAccessToken, process.env.JWT1);
